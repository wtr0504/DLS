...
__global__ void MatmulKernel(const float *a, const float *b, float *out,
  ...
  __shared__ float As[TILED][TILED + 1];
  __shared__ float Bs[TILED][TILED + 1];

  int tidr = threadIdx.y;
  int tidc = threadIdx.x;
  int row = blockIdx.y * TILED + tidr;
  int col = blockIdx.x * TILED + tidc;

  float res = 0.0f;

  for (int k = 0; k < N; k += TILED) {
    if (row < M && k + tidc < N)
      As[tidr][tidc] = a[row * N + k + tidc];
    else
      As[tidr][tidc] = 0.0f;

    if (col < P && k + tidr < N)
      Bs[tidr][tidc] = b[(k + tidr) * P + col];
    else
      Bs[tidr][tidc] = 0.0f;
