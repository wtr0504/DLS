...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        S = self.matmul(q,k) / (q_dim ** 0.5)
        if self.causal:
          mask = Tensor(self.create_causal_mask(queries_len,keys_values_len,device=self.device),device=self.device)
          S += mask.broadcast_to(S.shape)
        probs = self.dropout(self.softmax(S))

        return self.matmul(probs,v.transpose((2,3))) ,probs
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        q_ = self.q_projection(self.prenorm_q(q.reshape((batch_size*q_len,q_dim)))).reshape((batch_size,q_len,self.num_head,self.dim_head))
        k_ = self.k_projection(self.prenorm_k(k.reshape((batch_size*kv_len,k_dim)))).reshape((batch_size,kv_len,self.num_head,self.dim_head))
        v_ = self.v_projection(self.prenorm_v(v.reshape((batch_size*kv_len,v_dim)))).reshape((batch_size,kv_len,self.num_head,self.dim_head))

        q_ = q_.permute((0,2,1,3))
        k_ = k_.permute((0,2,1,3))
        v_ = v_.permute((0,2,1,3))

        x,probs = self.attn(q_,k_,v_) # x : B H T D
        self.probs = probs
        x = x.transpose((1,2)).reshape((batch_size,q_len,self.num_head * self.dim_head)).reshape((batch_size*q_len,self.num_head * self.dim_head))
        result = self.out_projection(x)

        return result.reshape((batch_size,q_len,self.out_features))
        # return result
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        ### BEGIN YOUR SOLUTION
        self.x_dim = q_features
        self.atten = AttentionLayer(q_features,num_head,dim_head,dropout=dropout,causal=causal,device=device,dtype=dtype)
        self.dropout = Dropout(dropout)
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        x = x + self.dropout(self.atten(x))
        x = x.reshape((batch_size*seq_len,x_dim))
        x = x + self.dropout(self.linear2(self.dropout(self.relu(self.linear1(self.layer_norm(x))))))
        ### END YOUR SOLUTION
        x = x.reshape((batch_size,seq_len,x_dim))
        return x
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transfomer_layers = [TransformerLayer(embedding_size,num_head,dim_head,hidden_size,dropout=dropout, causal=causal, device=device) for i in range(num_layers)]
        ### END YOUR SOLUTION
    ...
    def forward(
        ...
        pos = Tensor(np.arange(seq_len),device=self.device).reshape((seq_len,1))
        pos = self.embedding(pos).broadcast_to((seq_len, batch_size, embedding_size)).transpose((0,1))
        x = pos + x # b s d
        for m in self.transfomer_layers:
          x = m(x)
        ### END YOUR SOLUTION
