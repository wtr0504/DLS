...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        # if not (prod(new_shape) == self.size and self.is_compact()):
        #   breakpoint()
        # assert prod(new_shape) == self.size and self.is_compact()
        return self.make(shape=new_shape,device=self.device,handle=self._handle,offset=0)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        return self.make(shape=new_shape,strides=tuple(new_strides),device=self.device,handle=self._handle,offset=0)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        i = len(new_shape) - 1
        new_strides = []
        for l,s in zip(reversed(self.shape),reversed(self.strides)):
          if l == 1:
            new_strides.append(0)
          elif l != 1:
            assert l == new_shape[i]
            new_strides.append(s)
          i-=1
        if len(new_strides) < len(new_shape):
          new_strides.append(0)
        new_strides = tuple(reversed(new_strides))
        return NDArray.make(shape=new_shape,strides=new_strides,device=self.device,handle=self._handle,offset=0)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        for i,sl in enumerate(idxs):
          ...
          new_offset += start * self._strides[i]
        return NDArray.make(
    ...
    def flip(self, axes):
        ...
        for axis in axes:
          offset += new_strides[axis] * (self.shape[axis] - 1)
          new_strides[axis] = -self.strides[axis]
        return NDArray.make(self.shape,tuple(new_strides),device=self.device,handle=self._handle,offset=offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i,(l,r) in enumerate(axes):
          new_shape[i] += l + r
        slices = tuple(slice(axes[i][0],axes[i][0] + self.shape[i])
                      for i in range(len(self.shape)))
        array = full(tuple(new_shape),0,device=self.device)
        array[slices] = self
        return array
        ### END YOUR SOLUTION
