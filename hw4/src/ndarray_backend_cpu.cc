...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  /// BEGIN SOLUTION
  uint32_t cnt = 0;
  std::vector<int32_t> idx(shape.size());
...
void EwiseMaximum(const AlignedArray& a, AlignedArray& b, AlignedArray* out) {
  /**
   * Set entries in out to be the sum of corresponding entry in a plus the scalar val.
   */
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = std::max(b.ptr[i],a.ptr[i]);
  }
}
...
void EwiseEq(const AlignedArray& a, AlignedArray& b, AlignedArray* out) {
  /**
   * Set entries in out to be the sum of corresponding entry in a plus the scalar val.
   */
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = static_cast<scalar_t>(b.ptr[i] == a.ptr[i]);
  }
}
...
void EwiseGe(const AlignedArray& a, AlignedArray& b, AlignedArray* out) {
  /**
   * Set entries in out to be the sum of corresponding entry in a plus the scalar val.
   */
  for (size_t i = 0; i < a.size; i++) {
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for(int i = 0;i < out->size;i++){
    ...
    for(int j = 0;j < reduce_size;j++){
     max = std::max(a.ptr[i * reduce_size + j],max);
    }
    out->ptr[i] = max;
  }
  /// END SOLUTION
